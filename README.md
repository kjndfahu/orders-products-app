...
При старте backend контейнера выполняются следующие шаги (определены в `CMD` Dockerfile):

1. **`npx prisma migrate deploy`** — применяет все неприменённые миграции к БД
   - Идемпотентная операция: безопасна для повторного запуска
   - Проверяет таблицу `_prisma_migrations` и применяет только новые миграции
   
2. **`npm run prisma:seed || true`** — загружает тестовые данные из `seed.sql`
   - Выполняется только если БД пустая (проверка внутри скрипта)
   - `|| true` гарантирует, что ошибка seed не остановит запуск приложения
   
3. **`npm run start`** — запускает скомпилированный Express сервер (`dist/index.js`)

**Преимущества этого подхода:**
- 🔄 Автоматическая синхронизация схемы БД при обновлении кода
- 🚀 Не требует ручного запуска миграций после git pull
- 🐳 Контейнер полностью самодостаточен
- 🔒 Healthcheck MySQL гарантирует, что БД готова перед миграциями
...
При первом старте backend выполняет миграции Prisma и загрузку seed-данных автоматически (команда встроена в Dockerfile).

> **Важно:** Backend контейнер ждёт, пока MariaDB станет здоровым (healthcheck), прежде чем запуститься. Это гарантирует, что миграции не упадут из-за недоступности БД.
...
> **Примечание:** Миграции запускаются автоматически через `CMD` в Dockerfile каждый раз при старте контейнера. Команда `prisma migrate deploy` безопасна для повторного запуска — она применяет только новые миграции.
